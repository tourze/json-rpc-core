 ...
 * @internal
 */
#[CoversClass(ResponseSendingEvent::class)]
final class ResponseSendingEventTest extends TestCase
{
    ...
    public function testEventWithErrorResponse(): void
    {
        ...
        $callResponse = new JsonRpcCallResponse();
        $response = new JsonRpcResponse();
        $error = new JsonRpcMethodNotFoundException('test.method', [
            'method' => 'test.method',
        ]);
        ...
        $error = $event->getJsonRpcCallResponse()->getResponseList()[0]->getError();
        $this->assertNotNull($error);
        $this->assertEquals(-32601, $error->getErrorCode());
    }
    ...
    public function testEventDocumentation(): void
    {
        ...
        $this->assertStringContainsString('在响应已被端点成功序列化并即将返回时分发', $docComment);
    }
